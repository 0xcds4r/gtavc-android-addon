...
void crashdump::report(ucontext* context, siginfo_t *info, const char* typeName)
{
	...
	client_log.push(LOG_CAT_TAG, "Crash ID: %d", rand() % 99999);
	
	if(ms_uiInCrashZone != 0) {
		client_log.push(LOG_CAT_TAG, "Crash zone ID: %d", ms_uiInCrashZone);
	}

	client_log.push(LOG_CAT_TAG, "SIGNO: %s | Fault address: 0x%X", typeName, info->si_addr);
	client_log.push(LOG_CAT_TAG, "android_sigaction signal %d (si_code: %d si_errno: %d si_signo: %d)", info, info->si_code, info->si_errno, info->si_signo);

	if(!pGTAVC || !pIMM || !pCSysLibrary || !pSTDCPPSysLibrary || !pMSysLibrary || !pClientLibrary) {
		return;
	}

	client_log.push(LOG_CAT_TAG, "libGTAVC.so base address: 0x%X", pGTAVC);
	client_log.push(LOG_CAT_TAG, "libImmEmulatorJ.so base address: 0x%X", pIMM);
	client_log.push(LOG_CAT_TAG, "libc.so base address: 0x%X", pCSysLibrary);
	client_log.push(LOG_CAT_TAG, "libstdc++.so base address: 0x%X", pSTDCPPSysLibrary);
	client_log.push(LOG_CAT_TAG, "libm.so base address: 0x%X", pMSysLibrary);
	client_log.push(LOG_CAT_TAG, "libmultiplayer.so base address: 0x%X", pClientLibrary);

	client_log.push(LOG_CAT_TAG, "Lib crashed at:");
	...
	if(contextResult[0] <= 0xFFFFFF && contextResult[1] <= 0xFFFFFF) {
		client_log.push(LOG_CAT_TAG, "[system / libc.so] pc: 0x%X | lr: 0x%X", contextResult[0], contextResult[1]);
	}

	if(contextResult[2] <= 0xFFFFFF && contextResult[3] <= 0xFFFFFF) {
		client_log.push(LOG_CAT_TAG, "[system / libstdc++.so] pc: 0x%X | lr: 0x%X", contextResult[2], contextResult[3]);
	}

	if(contextResult[4] <= 0xFFFFFF && contextResult[5] <= 0xFFFFFF) {
		client_log.push(LOG_CAT_TAG, "[system / libm.so] pc: 0x%X | lr: 0x%X", contextResult[4], contextResult[5]);
	}

	if(contextResult[6] <= 0xFFFFFF && contextResult[7] <= 0xFFFFFF) {
		client_log.push(LOG_CAT_TAG, "[GAME / libGTAVC.so] pc: 0x%X | lr: 0x%X", contextResult[6], contextResult[7]);
	}

	if(contextResult[8] <= 0xFFFFFF && contextResult[9] <= 0xFFFFFF) {
		client_log.push(LOG_CAT_TAG, "[Emulator / libImmEmulatorJ.so] pc: 0x%X | lr: 0x%X", contextResult[8], contextResult[9]);
	}

	if(contextResult[10] <= 0xFFFFFF && contextResult[11] <= 0xFFFFFF) {
		client_log.push(LOG_CAT_TAG, "[Engine / libmultiplayer.so] pc: 0x%X | lr: 0x%X", contextResult[10], contextResult[11]);
	}
	...
	client_log.push(LOG_CAT_TAG, "r0: 0x%X, r1: 0x%X, r2: 0x%X, r3: 0x%X", context->uc_mcontext.arm_r0, context->uc_mcontext.arm_r1, context->uc_mcontext.arm_r2, context->uc_mcontext.arm_r3);
  	...
  	client_log.push(LOG_CAT_TAG, "r8: 0x%x, r9: 0x%x, r10: 0x%x, fp: 0x%x", context->uc_mcontext.arm_r8, context->uc_mcontext.arm_r9, context->uc_mcontext.arm_r10, context->uc_mcontext.arm_fp);
